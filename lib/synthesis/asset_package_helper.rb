...
module Synthesis
  module AssetPackageHelper
    def should_merge?
      AssetPackage.merge_environments.include?(Rails.env) || ENV.fetch("PAM_PACK_ASSETS", nil)
    end
    ...
    def javascript_include_merged(*sources)
      ...
      if sources.include?(:defaults)
        sources = sources[0..(sources.index(:defaults))] +
                  %w[prototype effects dragdrop controls] +
                  (File.exist?("#{Rails.root}/public/javascripts/application.js") ? ["application"] : []) +
                  sources[(sources.index(:defaults) + 1)..sources.length]
        sources.delete(:defaults)
      end

      sources.collect!(&:to_s)
      sources = (if should_merge?
